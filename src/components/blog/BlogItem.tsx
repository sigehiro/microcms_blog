        ...
        <Link href={`/blog/${blog.id}`} className="border group">
            <div className="aspect-video relative overflow-hidden">
                ...
                {isRanking && blog.ranking && (
                    <div className="absolute top-0 right-0 bg-white py-2 px-3 font-bold">{blog.ranking}</div>
                )}
            </div>

            <div className="p-5 space-y-5 hover:text-gray-500">
                <div>{blog.title}</div>
                <div className="text-xs text-gray-500">{format(new Date(blog.createdAt), 'yyy/MM/dd')}</div>
            </div>
        </Link>
    )
}
