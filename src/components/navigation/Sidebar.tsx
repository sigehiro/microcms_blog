...
const fetchSidebarData = async (): Promise<SidebarData> => {
    ...
    const extractArchiveMonths = (blogs: BlogType[]): ArchiveMonthType[] => {
        ...
        return Array.from(monthCounts.values()).sort((a, b) => b.year - a.year || b.month - a.month)
    }
        ...
        <div className="space-y-10">
            <div className="border flex flex-col items-center justify-center p-5 space-y-5">
                ...
                <div className="text-sm">
                    Canada で Computer Programming を学ぶ学生です！
                    <br />
                        ...
                        archiveMonths.map((archive, index, array) => {
                            return (
                                <Link
                                    ...
                                    key={index}
                                >
                                    <div>{format(new Date(archive.year, archive.month - 1), 'yyy年MM月')}</div>
                                    <div className="border py-1 px-4 text-sm">{archive.count}</div>
                                </Link>
                            )
                        })
                    )}
